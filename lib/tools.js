...
export const TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        function: {
            ...
            description: "Get system information including CPU, RAM, disk usage, installed packages, and running processes",
            parameters: {
                type: "object",
                properties: {},
                required: []
...
export class ToolExecutor {
    ...
    executeTool(toolName, parameters, callback) {
        let endpoint = "";
        let url = "";
        console.log('[ToolExecutor] Calling tool:', toolName, 'URL will be:', this._toolServerUrl);
            ...
            (session, result) => {
                try {
                    if (message.get_status() === Soup.Status.OK) {
                        ...
                        callback(new Error(`HTTP ${message.get_status()}: ${message.get_reason_phrase()}`), null);
                    }
                } catch (error) {
                    callback(error, null);
                }
