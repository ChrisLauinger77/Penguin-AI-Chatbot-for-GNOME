...
import { MessageRoles, CSS } from "./constants.js";
...
export class ChatMessageDisplay {
  ...
  constructor(container, styleSettings, onSettingsRequested, UI) {
  ...
  displayMessage(role, text) {
    ...
    const messageBoxType = isUserMessage
      ? CSS.HUMAN_MESSAGE_BOX
      : CSS.LLM_MESSAGE_BOX;
    ...
    //const formattedText = isUserMessage ? convertMD(text) : convertMD(text);
  ...
  displayError(errorMessage, showSettingsButton = true) {
    ...
    if (showSettingsButton) {
      const settingsButton = new St.Button({
        label: this._UI.SETTINGS_BUTTON_TEXT,
        can_focus: true,
        toggle_mode: true,
      });
  ...
  _addCopyBehavior(label, clipboard, chatInput) {
    ...
    label.connect("enter-event", async () => {
      showTooltip(this._UI.CLICK_TO_COPY_TOOLTIP);
    });
  ...
  _scrollToBottom() {
    ...
    this._scroll = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 50, () => {
      ...
      if (scrollView instanceof St.ScrollView) {
        ...
        if (vscroll) {
          vscroll.adjustment.value =
            vscroll.adjustment.upper - vscroll.adjustment.page_size;
        }
      } else {
